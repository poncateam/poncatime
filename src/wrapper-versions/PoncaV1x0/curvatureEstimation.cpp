...
struct NumNeighborHelper : public T
{
    ...
    int getNumNeighbors() { return Base::m_nbNeighbors; }
};

using NF        = DistWeightFunc<MyPointSimple, SmoothWeightKernel<double> > ;
using ASOBasket = Basket<MyPointSimple, NF, OrientedSphereFit, NumNeighborHelper>;
using ASOFit    = BasketDiff<ASOBasket, FitSpaceDer, OrientedSphereDer, MlsSphereFitDer>;
using PlaneFit  = Ponca::Basket<MyPointSimple, NF, CovariancePlaneFit, NumNeighborHelper>;
